...
```{r echo = F, warning = F, message=FALSE, warning=FALSE, packages}
...
```{r echo = F, warning = F, Load_the_Data}
# Load the Data
df <- read.csv('prosperLoanData.csv')
```

```{r echo=TRUE, Profile_the_Data}
...
```{r, echo=F, warning=F, ProsperScore_hist}
...
```{r, echo=F, warning=F, CreditGrade_hist}
...
```{r, echo=F, warning=F, CreditGrade_hist2, warning=F}
...
```{r, echo = F, warning = F, LoanOriginalAmount_summary}
## 
...
```{r echo = F, warning = F, LoanOriginalAmount_hist}
...
```{r echo = F, warning = F, loan_original_amount_hist}
...
## Income
```{r, echo = F, warning = F, inome_range_trans}
...
```{r, echo=F, warning=F, income_range_hist}
...
```{r, echo = F, warning = F, DebtToIncomeRatio_hist}
...
```{r, echo=T, StatedMonthlyIncome_summary}
...
```{r, echo = F, warning = F, EmploymentStatus_trans}
...
```{r, echo = F, warning = F, employment_status_hist, fig.width=15}
...
```{r, warning = F, LenderYield_hist2}
qplot(x = LenderYield, data = df)
```


```{r, warning = F, estimated_effective_yield_hist}
qplot(x = EstimatedEffectiveYield, data = df)
```

```{r, warning = F, estimated_effective_yield_hist2}
qplot(x = EstimatedEffectiveYield, data = df, binwidth = 0.0025)
```


```{r, warning = F, estimated_loss_hist}
qplot(x = EstimatedLoss, data = df)
```

```{r, warning = F, estimated_loss_hist2}
qplot(x = EstimatedLoss, data = df, binwidth =0.0025)
```

```{r, warning = F, estimated_return_hist}
...
```{r, echo = F, warning = F, estimated_return_hist2}
...
```{r, TotalProsperPaymentsBilled_hist}
qplot(x = TotalProsperPaymentsBilled, data = df)
```

```{r, TotalProsperPaymentsBilled_hist2}
qplot(x = TotalProsperPaymentsBilled, data = df,
      binwidth = 1) + 
...
```{r, TotalProsperPaymentsBilled_data}
head(na.omit(df$TotalProsperPaymentsBilled), 1000)
```


```{r, TotalProsperPaymentsBilled_summary}
...
```{r, echo = F, warning = F, date_conversion}
...
```{r, echo = F, warning = F, CreditLines_hist}
grid.arrange(
...
```{r, echo = F, warning = F, Delinquencies_hist}
grid.arrange(
...
```{r, echo = F, warning = F, loan_status_hist}
...
## Correlaion
```{r, echo=F, warning=F, corr_helper}
...
```{r, echo=F, warning=F, correlation_test}
...
```{r, echo = F, warning = F, BivariatePlots_helper}
...
```{r, echo = F, warning = F, PS_v_Income1, fig.width=10}
grid.arrange(
...
```{r, echo=F, warning=F, PS_v_BorrowerAPR_scatter}
...
```{r, echo=F, warning=F, PS_v_BorrowerAPR_hist, fig.width=12}
ggplot(data = df) + 
...
The above plots confirms that there are quite many loans (ProsperScore <= 5) are at BorrowerAPR ~ 0.35. What are they? Also, for loans with ProsperScore in 6-7, the BorrowersAPR distribution appears to be bimodal or multimodal peaking at 0.2 and 0.3.

```{r, echo=F, warning=F, PS_v_BorrowerAPR_oddity}
...
```{r, echo=F, warning=F, PS_v_EstimatedReturn_scatter}
...
```{r echo = F, warning = F, PS_v_IncomeRange}
...
```{r echo = F, warning = F, PS_v_StatedMonthlyIncome}
...
```{r, echo=F, warning=F, PS_v_StatedMonthlyIncome2}
ggplot(data = df,
...
```{r, echo = F, warning = F, PS_v_StatedMonthlyIncome3}
...
```{r, echo = F, warning = F, PS_v_DebtToIncomeRatio}
...
```{r, echo = F, warning = F, PS_v_CreditHistory}
grid.arrange(
...
```{r, echo = F, warning = F, PS_v_CreditHistory2}
grid.arrange(
...
```{r, echo=F, warning=F, PS_v_LoanOriginalAmount_box}
...
```{r, echo=F, warning=F, BorrowerAPR_v_LoanOriginalAmount_scatter}
...
```{r, echo=F, warning=F, cut_LoanOriginalAmount_trans}
...
```{r, echo=F, warning=F, categorize_ps}
df$ProsperScore.bucket <- cut(df$ProsperScore, breaks = c(0, 2, 5, 9, 11))

```

```{r, echo=F, warning=F, PS_v_BorrowerAPR_IncomeRange, fig.width=10}
ggplot(data = df) + 
...
```{r, echo=F, warning=F, PS_v_BorrowerAPR_CreditScoreRangeLower, fig.width=10}
ggplot(data = df) + 
...
```{r, echo=F, warning=F, PS_v_BorrowerAPR_CreditScoreRangeLower2, fig.width=10}
...
```{r, echo=F, warning=F, PS_v_EstimatedEffectiveYield, fig.width=10}
ggplot(data = df) + 
...
```{r, echo=F, warning=F, PS_v_LenderYield, fig.width=10}
ggplot(data = df) + 
...
```{r, echo=F, warning=F, interest_var_scatter_matrix, fig.width=12, fig.height=12}
...
The results of the model seems to be quite bad as it only predicts 17% of the correct ProsperScore. It doesn't predict correctly for loans with ProsperScore over 9 and equal to 1.
...
### Plot One
```{r echo = F, warning = F, Plot_One, fig.width=10, fig.height=10}
# create labels for facted plots
df$ProsperScore.label <- sapply(df$ProsperScore, function (x) {
  paste('ProsperScore = ', x)
})
...
### Description One
The distribution of BorrowerAPR shifts from right to left with increase of ProsperScore, which indicates a negative correlation between ProsperScore and BorrowerAPR. It also shows there are a few peak values with BorrowerAPR near 0.36 for ProsperScore within 2-5. 


### Plot Two
```{r echo = F, warning = F, Plot_Two, dpi=200, fig.width=12, fig.height=8}
# main plot
...
### Description Two
StatedMonthlyIncome tends to be higher with increase of the Prosper Score except for ProsperScore of 1 which has higher income than Score 2 and even 5. 


### Plot Three
```{r echo = F, warning = F, Plot_Three, dpi=200, fig.width=12, fig.height=8}
...
### Description Three
The number of Prosper loans originated increase since 2006 and stumbled to 0 since end of 2008 as a result of the 2008 Global Finacial Crisis. Since the year end of 2009, the loans started to be originated again and climbed to 100 per day during the 3rd quarter in 2012 then dropped to 50 due to 2012 Financial Crisis. Loans originated per day then had a significant jump from 50 to over 250 in 2013.
